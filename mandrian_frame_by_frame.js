...
const DELAY = 100; // Temps en millisecondes qui va séparer les affichages des différentes couches de rectangles pour créer une animation au chargement de la page
...
// Ici elle est asynchrone car nous utilisons une promesse pour attendre un petit laps de temps entre les opérations et créer une animation au début du programme
async function draw() {
    // On définit la couleur de fond de l'oeuvre
    background(BACK_COLOR);

    // On déclare et donne une valeur aléatoire bornée à une variable qui va définir le nombre de rectangles à créer
    let depth = Math.floor(random(MIN_DEPTH, MAX_DEPTH));
    ...
    for (let i = 0; i < nbIterations; i++) {
        await waitTime(DELAY); // on attend 100 millisecondes avant d'exécuter la prochaine opération (crée l'animation d'apparition au chargement de la page)
        drawRect(0, 0, CANVA_WIDTH, CANVA_HEIGHT, depth, depth, colors); // on récupère tous les éléments qui ont la classe "click" (ce sont des div qui sont créées par dessus chaque rectangles pour que l'on puisse cliquer dessus et ajouter un évènement lors de ce clic). Ainsi on peut simuler une action lors d'un clic sur un rectangle.
    }
}
...
// Ainsi chaque rectangle crée lui-mÃªme deux autres rectangles au fur et à mesure, ce qui diminue la taille
...
// Cette fonction permet d'attendre un certain délai avant d'exécuter la prochaine opération du programme
// Elle crée une promesse (une valeur à retourner au client qui peut être disponible maintenant, bientôt, ou jamais)
// Ainsi si on appelle cette fonction, il faudra l'exécuter de manière asynchrone, c'est à dire que le navigateur aura
// l'information qu'une tâche de durée incertaine va s'exécuter, et donc ne va pas attendre de réponse du serveur avant de continuer le programme.
// Ainsi on peut en quelque sorte dire au navigateur qu'une tâche longue (ici l'attente d'un certain nombre de millisecondes) va être exécutée sans le prendre au dépourvu
